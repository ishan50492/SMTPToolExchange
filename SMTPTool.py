...
def main():
    ...
    parser.add_argument("-q", "--quantity", type=int, dest="quantity", help="Number of emails to be generated",
                      metavar="n")
    parser.add_argument("-r", "--dryrun", action="store_true", dest="dryrun", help="Execute script without sending email")
    ...
    parser.add_argument("-o", "--jsonoutputfile", action="store", type=str, dest="json_output_path",
    ...
    parser.add_argument("-d", "--domainname", action="store", type=str, dest="domain_name",
                      help="Adds/replaces domains of provided email addresses", metavar="domainname")
    parser.add_argument("-m", "--smtp", nargs=2, dest="addresses", help="Use Mail Server for mail transfer", metavar="addresses")
    ...
    if args.addresses is None and args.exc is False:
        print("Atleast one of Exchange or Mail Server should be selected")
        return

    if args.addresses is not None and args.exc is not False:
        print("Only one of Exchange or Mail Server should be selected")
        return
    ...
    # If Exchange or Mail Server is used to send mail, and external file is provided, use that file
...
def interf(args):
    if (args.json_input == ""):
        print('Generating ' + str(args.quanity) + ' emails...')
        mail_random_emails(args)
    else:
        print('Sending emails from', args.json_input)
        mail_input_emails(args)
